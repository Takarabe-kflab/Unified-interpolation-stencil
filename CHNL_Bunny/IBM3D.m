...
for uvw = 1:3   %for making 3 kernels for u,v,w to fit the staggered grid
    ...
    if uvw == 1
        fname = "kernel/GP_weights_u.txt";
        input_file = "sdf/SDF_xshift.csv";
    elseif uvw == 2
        fname = "kernel/GP_weights_v.txt";
        input_file = "sdf/SDF_yshift.csv";
    elseif uvw == 3
        fname = "kernel/GP_weights_w.txt";
        input_file = "sdf/SDF_zshift.csv";
    end
    ...
    sdf_data=readtable(input_file);
    phi = zeros(Nx,Ny,Nz);
    for p = 1:height(sdf_data)
        ix = sdf_data.grid_x(p);
        iy = sdf_data.grid_y(p);
        iz = sdf_data.grid_z(p);
        phi(ix, iy, iz) = sdf_data.sdf(p);
    end
